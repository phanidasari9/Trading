...
def write_swing_scan_excel(writer: pd.ExcelWriter, stock_df: pd.DataFrame, contract_df: pd.DataFrame) -> None:
...
def save_results_to_excel(stock_df, contract_df, filename):
    with pd.ExcelWriter(filename, engine="openpyxl") as writer:
        write_swing_scan_excel(writer, stock_df, contract_df)


def swing_results_to_excel_bytes(stock_df: pd.DataFrame, contract_df: pd.DataFrame) -> bytes:
    buf = io.BytesIO()
...
def format_swing_display_df(df: pd.DataFrame) -> pd.DataFrame:
...
def run_swing_scan(symbols: list[str], *, top_stocks_for_options: int | None = None) -> tuple[dict, pd.DataFrame]:
    """
    Back-compat: returns (market_context, stock_df only).
    Prefer ``run_swing_full_scan`` for contracts + Excel.
    """
    mc, stock_df, _ = run_swing_full_scan(symbols, top_stocks_for_options=top_stocks_for_options)
    return mc, stock_df


def run_swing_full_scan(
    symbols: list[str],
    *,
    top_stocks_for_options: int | None = None,
) -> tuple[dict, pd.DataFrame, pd.DataFrame]:
    """
    Run stock setup scan + best option contracts for top CALL/PUT names.
...
def swing_scan_to_excel_bytes(df: pd.DataFrame) -> bytes:
    """Legacy single-sheet helper: stock setups only (empty contracts sheet)."""
    return swing_results_to_excel_bytes(df, pd.DataFrame())
